...
REPLACEMENT_REGEXES_STR = [
    ...
    # main replacement(s)
    (r'(\b)chrome://', r'\1hydrogen://'),
    (r'(?:Google )?Chrom(e|ium)(?!\w)', r'Hydrogen'),
...
def replace_text(text):
    """Replaces instances of Chrom(e | ium) with Hydrogen in strings"""
    had_match = False
...
def replace_grit_message(msg):
    """
    Replaces instances of Chrom(e | ium) with Hydrogen
...
def replace_xtb_translation(msg, fp_map):
    """
    Replaces instances of Chrom(e | ium) with Hydrogen in a
    .xtb <translation>.
    """
...
def replace_grit_tree(text):
    """Replaces instances of Chrom(e | ium) with Hydrogen, where desired"""
...
def dedup_translations_in_place(translation, seen):
    ...
    # "Chrome" variant. Since we substituted both of those with Hydrogen,
