...
# Copyright 2025 The Hydrogen Authors
# You can use, redistribute, and/or modify this source code under
# the terms of the GPL-3.0 license that can be found in the LICENSE file.
"""
Script to generate Hydrogen version numbers
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "HYDROGEN" in file.read():
            raise ValueError("file already contains hydrogen versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "HYDROGEN_MAJOR": tree / "version.txt",
        "HYDROGEN_MINOR": tree / "chromium_version.txt",
        "HYDROGEN_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["HYDROGEN_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "HYDROGEN_MINOR" else -CHROME_VERSION_BASE
